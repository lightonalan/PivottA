...
class DatePicker extends Component {
  ...
  getDate(date = this.props.date) {
    ...
    // date默认值
    if (!date) {
      let now = new Date(customDefaultDate);
      if (minDate) {
  ...
  getDateStr(date = this.props.date) {
    const {
      isQualification,
      mode,
      format = isQualification ? 'YYYY / MM' : FORMATS[mode],
    } = this.props;
  ...
  getTitleElement() {
    const {isQualification, date, placeholder, customStyles, allowFontScaling} =
      this.props;
    ...
    if (!date && placeholder) {
      return (
        <Text
          ...
          {isQualification ? null : <Text style={{color: 'black'}}>{'/'}</Text>}{' '}
